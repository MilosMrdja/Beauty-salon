...
import GUI.RecepcionerTableFrame;
...
public class RecepcionerAddEdit extends JDialog {

	private static final long serialVersionUID = -8035045826728283445L;
	private KorisniciKontroler korisniciKontroler;
	private JTextField bonusTxt;
	private JTextField plataTxt;

	public RecepcionerAddEdit(JFrame p,KorisniciKontroler k, Recepcioner r) {
		if(r == null) {
			setTitle("Dodavanje recepcionera");
		}
		else {
		...
		getContentPane().setLayout(new MigLayout("", "[100,grow][100,grow][100,grow][100,grow]", "[50][50][50][50][50][50][50][50.00][50][50.00][50][70.00][50.00]"));
		...
		if(r != null) {
			...
			imeTxt.setText(r.getIme());
			prezimeTxt.setText(r.getPrezime());
			telefonTxt.setText(String.valueOf(r.getTelefon()));
			adresaTxt.setText(r.getAdresa());
			korImeTxt.setText(r.getKorisnickoIme());
			korImeTxt.setEditable(false);
			lblKorisnickoIme.setText("Korisnicko ime: (Ne menja se)");
			lozinkaTxt.setText(r.getLozinka());
			stazTxt.setText(String.valueOf(r.getRadniStaz()));
			osnovaPlateTxt.setText(String.valueOf(r.getOsnovaPlate()));
			bonusTxt.setText(String.valueOf(r.getBonus()));
			plataTxt.setText(String.valueOf(r.getPlata()));
			if(r.getStrucnaSprema() == 0.2) {
				comboBox.setSelectedIndex(0);
			}
			else if(r.getStrucnaSprema() == 0.4) {
				comboBox.setSelectedIndex(1);
			}
			else if(r.getStrucnaSprema() == 0.6) {
				comboBox.setSelectedIndex(2);
			}
			else if(r.getStrucnaSprema() == 0.8) {
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				p.setEnabled(true);
				RecepcionerAddEdit.this.dispose();
		...
		btnDodaj.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(flag == true) {
					...
					if(r == null) {
						k.registrujRecepcionera(new Recepcioner(imString, preString, broj, adresaString, korString, passString, sprema, rs, op, 0));
					}
					else {
						r.setIme(imString);
						r.setPrezime(preString);
						r.setTelefon(broj);
						r.setAdresa(adresaString);
						r.setKorisnickoIme(korString);
						r.setLozinka(passString);
						r.setRadniStaz(rs);
						r.setOsnovaPlate(op);
						r.setStrucnaSprema(ss);
						korisniciKontroler.azurirajFile();
					}
					((RecepcionerTableFrame)p).refreshData();
					p.setEnabled(true);
					RecepcionerAddEdit.this.dispose();
				}
			}
